...
Занимаюсь ИТ профессионально с 1994г, специализируюсь на бэкенде, но имею опыт в администрировании linux и задачах фронтенда. Не ограничиваюсь кодированием по ТЗ и стараюсь делать все возможное для успеха проекта в целом.

Не считаю правильным приходить в ИТ начальником. Сначала надо стать частью команды, а там видно будет. Статистика говорит, что я могу быстро дойти до CTO, но, возможно, выборка маленькая, т.к. я присутствую в штате или меньше 2х недель или больше 3х лет.

Индивидуальный предприниматель на ПСН, но не фрилансер. Участвую в проектах, которые требуют несколько месяцев и полное погружение. Для дедлайнов могу находить до 16ч в сутки, но недолго.

В свободное время и в рамках проектов [участвую в opensource](https://github.com/LeKovr).

## Что ищу

Команду профессионалов (поэтому прошу ссылки на github) с вменяемыми менеджерами (способными отличить рейтинг новичка/говоруна/дармоеда от рейтинга проверенного кадра), которая
...
* проекты по распилу денег инвесторов, "NoSQL" и "BTC"
...
6. К моменту перехода проекта в стабильный продакшен с обновлениями, у него уже есть команда специалистов, способных его поддерживать без разрушения функциональности, и я могу переключиться на следующий проект
...
* SQL (Oracle: 1997 - 1999, PostgreSQL: 1999 - н.в.)
* golang (2015 - н.в.)
* linux (1998 - н.в., перестал использовать другие ОС в 2002)
* docker ([2014](https://github.com/LeKovr/consup/commit/0decc256f3ae5c6ae057c398105f0e1ec20dc591) - н.в.)
* javascript (2002 - н.в.)
* perl (1999 - 2015)
* PAL/QBE (1994 - 1997)
* [C, Modula, Clarion](https://kovrizhkin.jast.ru/prj/ksu93.html) (1993 - 1994)
* Pascal (1990 - 1993)
...
Являясь техлидом проекта [tender.pro](https://www.tender.pro/index.shtml) с 2009г, с 2015г продолжаю выполнять эти функции на договорной основе. Моя основная задача - обеспечивать функционирование существующего с 2002г проекта при постоянном наращивании функционала. Кроме этого, мною решались такие задачи, как:
* проектирование и реализация прототипов перспективной архитектуры проекта (в т.ч. [RPC-sample](https://github.com/TenderPro/rpc-sample-app))
* проектирование и реализация JSON<->SQL proxy для тендерной системы ([dbrpc](https://github.com/LeKovr/dbrpc))
* контейнеризация legacy-подсистем (в т.ч. [modperl](https://github.com/TenderPro/dockerfile-modperl))
* CI/CD на базе docker (см [dcape](https://github.com/dopos/dcape))

В личное время участвовал в проектах (в т.ч. опенсорс) по разработке ПО, использующих postgresql, golang, docker, make, js (см. ниже - "Проекты")
...
Чем я занимался с 2012 года, удобнее смотреть так:

* [codersrank](https://profile.codersrank.io/user/lekovr)
* [sourcerer](https://sourcerer.io/lekovr)

PS.  Рейтинг там смысла не имеет, т.к. выборка не репрезентативна и критерии примитивны.
...
* 2017, Бэкенд тендерной системы ([dbrpc](https://github.com/LeKovr/dbrpc), plpgsql)
...
Если вы прочитали этот текст и хотите написать мне письмо, имеет смысл добавить в заголовок слово "pogodo", что поможет письму не попасть в спам.
